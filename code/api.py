...
from code.predict import DecisionTreeModel
...
def predict() -> Response:
    """
    Predicts the usertype of the input data.
    ...
    message_df = pd.DataFrame(
        {key.replace('_', ' '): [value] for key, value in message.items()}
    )
    preprocessed_message = preprocessing.run(message_df, training=False)

    prediction = {config['dataset']['label_column']: model.predict(preprocessed_message)}
